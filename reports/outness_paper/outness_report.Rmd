...
```{r}
find_outness_relationship(school_affinity_group_involved)
```



```{r}
find_outness_relationship(out_classmates_peers)

out_plot(out_classmates_peers)
```

```{r}
find_outness_relationship(school_activities_advocacy)

out_plot(school_activities_advocacy)
```
...
model_data <- 
  ...
  select(
    ...
    #starts_with("so_"), 
    #starts_with("race_"), 
    starts_with("school_affinity"), 
    starts_with("school_activities"), 
    starts_with("satisfaction_"), 
    #state, 
    degree_type
  ) %>% 
  select(
    -ends_with("_description"), 
    -ends_with("explanation"), 
    -ends_with("_benefit")
  ) %>% 
  mutate(
...
outness_model <-
  fit_xy(
    outness_model, 
    y = pull(model_data, out_residency_app), 
    x = select(model_data, -out_residency_app), 
  )
