---
title: "Are LGBTQ+ people comfortable being \"out\" in medical school?"
...
In this analysis, we're going to clean and visualize survey data of 1162 medical students collected by the [Medical Student Pride Alliance (MSPA)](https://www.medpride.org/), a nonprofit organization that works with medical students who identify as lesbian, gay, bisexual, transgender, queer, or another sexual and/or gender minority identity (LGBTQ+). This survey was conducted as a part of MSPA's larger needs-assessment of LGBTQ+ medical students' needs and experiences while in medical school, but here we will focus explicitly on questions involving whether or not students were comfortable being "out" with regard to their sexual orientation and/or gender identity while in medical school. Many medical students report concerns about fear of discrimination while in medical school - and particularly when they apply to residency after medical school. So, it's important to know where LGBTQ+ medical students' are comfortable being "out" (and where they aren't) so that we can focus diversity, equity, and inclusion efforts in the right places. 
...
Note that working with the columns starting with `gender_`, `so_`, and `race_` is a little tricky because none of the categories are mutually-exclusive (someone can identify as both "gay" and "lesbian", for instance). So, even though we can describe variables encoding gender, sexual orientation, and race as categorical, we have to be careful when we compare them (it's not as easy as just setting up a single factor variable for gender, sexual orientation, and race!).  
...
gay <- 0.031
trans <- 0.003 
total_perc <- bi + gay + trans
estimated_total_enrollment_2019 <- 91247

# md respondents who are lgbtq+ 
lgbtq_res <- 
  na_data %>% 
  count(is_lgbtq, degree_type) %>% 
  filter(is_lgbtq == "LGBTQ+", degree_type == "allopathic") %>% 
  pull(n)

total_lgbtq_aamc <- total_perc * estimated_total_enrollment_2019

rr <- 
  (lgbtq_res / total_lgbtq_aamc) %>% 
  round(3)
...
Fourth, we can compute the how many MD vs. DO students were assigned male or female sex at birth. 
...
```{r}
my_footnote <- 
  "Because gender, sexual orientation, and race/ethnicity categories are not mutually exclusive, all \n percentages are reported relative to the total number of respondents in the study."

demo_combined <- 
  bind_rows(demo_sex, demo_breakdown, demo_year) %>% 
  pivot_longer(
    cols = c(count, percentage), 
  ...
  mutate(
    variable_type = 
      factor(
        variable_type, 
        levels = c("gender", "sex", "so", "race", "med_school_year")
      )
  ) %>% 
...
demo_table_text <- 
  ...
  bind_rows(demo_combined) %>% 
    transmute(
    variable = recode(variable, !!! recode_values),  
  ...
  # add row for Sex assigned at birth header
  add_row(
  ...
  # add row for sexual orientation header
  add_row(
  ...
  # add row for Race/Ethnicity header
  add_row(
  ...
  # add row for Year in School header
  add_row(
...
table_path <- 
  file.path("~", "Desktop", "demo_table.docx")
...
demo_table_text %>% 
  ...
  bold(part = "body", i = 1, j = 1:ncol(demo_table_text)) %>% 
  # insert gender header 
  bg(part = "body", i = c(2, 11, 14, 24, 32), bg = "#666666") %>% 
  color(part = "body", i = c(2, 11, 14, 24, 32), color = "#ffffff") %>% 
  bold(part = "body", i = c(2, 11, 14, 24, 32)) %>% 
  footnote(value = as_paragraph(my_footnote), ref_symbols = " ") %>% 
  autofit() %>% 
  save_as_docx(path = table_path)
...
demo_table <- 
  ...
  pack_rows("Sexual Orientation", 12, 20, label_row_css = "background-color: #666; color: #fff;") %>% 
  pack_rows("Race/Ethnicity", 21, 27, label_row_css = "background-color: #666; color: #fff;") %>% 
  pack_rows("Year in School", 28, 31, label_row_css = "background-color: #666; color: #fff;") %>% 
  add_footnote(
    label = my_footnote,
    notation = "number"
  )
...
q_test <- 
  ...
  CochranQTest(response ~ context | id, data = .) %>% 
  tidy()
...
overall_outness_plot %>% 
  ggsave(
    filename = "overall_outness_plot.jpg", 
    plot = ., 
    device = "jpg", 
    path = figure_out_path, 
    width = 6.5, 
    ...
    data = 
      map(
        ...
        .f = ~ 
          ...
          mutate(
            identity = 
              fct_lump(identity, prop = 0.005, w = n, other_level = "another") %>% 
              as.character()
          ) %>% 
          count(identity, wt = n)
      )
...
```{r}
gender_irrelevant_levels <- c("another", "gender-expansive", "man", "woman") 
...
gender_plot %>% 
  ggsave(
    filename = "gender_plot_supplemental.pdf", 
    plot = ., 
    device = "pdf", 
    path = figure_out_path, 
    width = 7, 
...
gender_plot %>% 
  ggsave(
    filename = "gender_plot_supplemental.jpg", 
    plot = ., 
    device = "jpg", 
    path = figure_out_path, 
    width = 7, 
...
```{r}
gender_irrelevant_levels <- c("man", "woman", "gender-expansive", "another") 
gender_subtitle <- "LGBTQ+ student \'outness\' by gender"
...
We can then delve into the SGM-specific and non-SGM-specific opinions about whether or not students should be able to be out. 

```{r}
num_lgbtq <- sum(na_data$is_lgbtq == "LGBTQ+", na.rm = TRUE)
...
support_plot_sgm <- 
  ...
  geom_text(
    aes(y = percent_out + 9, group = variable, label = significance),
    size = 6, 
    position = position_dodge(width = 0.91)
  ) + 
  scale_x_discrete(labels = function(x) str_wrap(x, width = 30)) + 
...
outness_data <- 
  ...
  mutate(
    is_sgm = 
      is_lgbtq %>% 
      fct_recode(SGM = "LGBTQ+", `Not SGM` = "Non-LGBTQ+")
  )
```
...
```{r}
# statistical testing 
# test proportion differences in supporting outness across 
# contexts between SGM and non-SGM people
support_comparison_chis <- 
...
```{r}
support_plot_comparison <- 
  outness_data %>% 
  filter(variable == "Support ability to be out") %>% 
  group_by(is_sgm, variable, context) %>%
  summarize(prop = sum(is_out == "yes") / n()) %>%
  mutate(
    environment = factor(context, levels = context_order), 
    is_sgm = factor(is_sgm, levels = c("SGM", "Not SGM")), 
    percent_out = (prop * 100) %>% round(1), 
    identity = "overall", 
    #significance = if_else(str_detect(variable, "or plan to be out"), "*", "")
  ) %>% 
  left_join(support_comparison_chis, by = c("context" = "environment", "is_sgm")) %>% 
  mutate(is_sgm = factor(is_sgm, levels = c("SGM", "Not SGM"))) %>% 
  ggplot(aes(x = environment, y = percent_out)) + 
  geom_col(
    aes(fill = is_sgm), 
    position = "dodge", 
  ...
  geom_text(
    aes(y = percent_out + 10, group = is_sgm, label = significance),
    size = 6,
    position = position_dodge(width = 0.9)
  ) +
  scale_x_discrete(labels = function(x) str_wrap(x, width = 30)) + 
  ...
  labs(
    subtitle = 
      "In medical school, in which contexts do you feel trainees should be able to be \"out\" with regard to their sexual orientation or gender identity?" %>% 
      str_wrap(width = 60),
    x = NULL, 
    y = "% of students", 
    fill = NULL,
...
```{r}
# statistical testing 
# test proportion differences in believing in protections across 
# contexts between SGM and non-SGM people
protections_comparison_chis <- 
  tibble(environment = context_order) %>% 
  mutate(
    chi_tests = 
      map(
        ...
        .f = ~ 
          table(
            outness_data %>%
              filter(context == .x) %>% 
              filter(variable == "Support protections for outness") %>% 
              pull(is_sgm), 
            outness_data %>%
              filter(context == .x) %>% 
              filter(variable == "Support protections for outness") %>% 
...
protections_plot_comparison <- 
  ...
  left_join(protections_comparison_chis, by = c("context" = "environment", "is_sgm")) %>% 
  mutate(is_sgm = factor(is_sgm, levels = c("SGM", "Not SGM"))) %>% 
  ggplot(aes(x = environment, y = percent_out)) + 
  geom_col(
    aes(fill = is_sgm), 
  ...
  labs(
    subtitle = 
      "In which contexts do you feel medical schools should have protections in place so that trainees can be \"out\" with regard to their sexual orientation/gender identity?" %>% 
      str_wrap(width = 60),
    x = NULL, 
    y = "% of students", 
    fill = NULL,
...
Significance testing for differences in number of lgbtq+ people "out" compared to how many want to be out/support protections for outness.

Using a simple `prop.test` test, we can also confirm that all of these gaps are significant (even the first one).

Among queer people, are more queer people supportive of their ability to be out than are actually out in each context? 
...
outness_vs_support_tests <- 
  ...
  unnest(chi_tests) %>% 
  mutate(p.value = p.value * 5) %>% 
  transmute(
    environment, 
    chi_squared = round(statistic, 2),
    # note the bonferonni correction in the line below
    p_value = formatC(p.value, digits = 1, format = "e"), 
    df = parameter, 
    significance = if_else(p.value < 0.05, "*", "") 
  )
...
alluvium_plot %>% 
  ggsave(
    filename = "outness_alluvium_2.jpg", 
    plot = ., 
    device = "jpg", 
    path = file.path(figure_out_path), 
    width = 4.5, 
...
figure_1 %>% 
    ggsave(
    filename = "figure_1_version1.jpg", 
    plot = ., 
    device = "jpg", 
    path = figure_out_path, 
    width = 11, 
...
figure_1 %>% 
    ggsave(
    filename = "figure_1_version2.jpg", 
    plot = ., 
    device = "jpg", 
    path = figure_out_path, 
    width = 7, 
...
alternate_figure_1 %>% 
    ggsave(
    filename = "figure_1_version3.jpg", 
    plot = ., 
    device = "jpg", 
    path = figure_out_path, 
    width = 13, 
...
mcnemar_test <- 
  function(my_context) { 
    outness_data %>% 
      filter(is_lgbtq == "LGBTQ+") %>% 
      filter(variable != "Support ability to be out") %>% 
      filter(context == my_context) %>% 
      select(variable, is_out) %>%  
      table() %>% 
      mcnemar.test() %>% 
      tidy()
  }
...
gap_tests <- 
  tibble(context = unique(outness_data$context)) %>% 
  mutate(test_results = map(.x = context, mcnemar_test)) %>% 
  unnest(test_results) %>% 
  mutate(p_value = pmin(1, p.value * 5))
...
```{r}
find_outness_relationship <- function(variable) {
  my_table <- 
    table(
      na_data %>% 
        filter(is_lgbtq == "LGBTQ+") %>% 
        pull(out_residency_app), 
      na_data %>% 
        filter(is_lgbtq == "LGBTQ+") %>% 
        pull({{variable}})
    )
  ...
  return(list(table = my_table, chi = chi_test))
}
```
...
```{r}
out_plot <- function(variable) {
  na_data %>% 
  filter(is_lgbtq == "LGBTQ+") %>% 
  count(out_residency_app, {{variable}}) %>% 
  drop_na() %>% 
  group_by(out_residency_app) %>% 
  mutate(prop = n / sum(n)) %>% 
  ggplot(aes(x = out_residency_app, y = prop, fill = {{variable}})) + 
  geom_col() + 
  coord_flip()
}
