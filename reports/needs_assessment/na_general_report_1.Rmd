...
We can appreciate this slightly better with a waffle visualization: 
...
However, we remember from our response plots above that most of the schools from which we have responses only have a few (<5) students who responded...this is a problem we will either have to solve or something that we will have to wordsmith around when we're trying to publish. 
...
```{r}
na_data %>%
  select(satisfaction_lgbtq_meded, is_lgbtq) %>% 
  drop_na() %>% 
  count(is_lgbtq, satisfaction_lgbtq_meded) %>% 
  group_by(is_lgbtq) %>% 
  mutate(group_percentage = n / sum(n) * 100)

na_data %>%
  select(satisfaction_lgbtq_meded, is_lgbtq) %>% 
  drop_na() %>% 
  count(satisfaction_lgbtq_meded) %>% 
  mutate(group_percentage = n / sum(n) * 100)

straight_mean <- 
  na_data %>% 
  group_by(is_lgbtq) %>% 
  summarize(satisfaction_mean = mean(satisfaction_lgbtq_meded, na.rm = TRUE)) %>% 
  filter(is_lgbtq == "Non-LGBTQ+") %>% 
  pull(satisfaction_mean)

na_data %>% 
  filter(is_lgbtq == "LGBTQ+") %>% 
  summarize(percent_lower = mean(satisfaction_lgbtq_meded < straight_mean, na.rm = TRUE))
